...
const AIChatScreen = () => {
    ...
    const pushMessage = useCallback((text: string, sender: 'user' | 'ai', extras?: Partial<ChatMessage>) => {
    ...
    const handleSend = useCallback(async () => {
        ...
        try {
            ...
            if (error.message === 'TIMEOUT') {
                pushMessage("It's taking a bit longer than usual to process. Could you try sending that again? 🐢", 'ai');
            } else {
                pushMessage("I'm having a little trouble understanding right now. Could you rephrase your last message? 😔", 'ai');
            }
        } finally {
            clearTimeout(timeoutId!);
            setIsLoading(false);
        }
    }, [inputText, isReady, isLoading, pushMessage, removeMessage, tripParams, setTripParams, setMessages]);
    ...
    const handleGeneratePlan = useCallback(async () => {
        ...
        if (!tripParams.destination || !tripParams.duration) {
            Alert.alert('Missing Info', 'Please provide destination and duration before generating plan.');
            return;
        }
    ...
    const renderSummaryCard = useCallback((item: ChatMessage) => {
                    ...
                    <View style={styles.inputContainer}>
                        ...
                        >
                            {inputText.length > 0 ? (
                                ...
                                <MaterialCommunityIcons name={"star-four-points" as any} size={22} color="#F9D423" />
                            )}
                        </TouchableOpacity>
                    </View>
                </View>
