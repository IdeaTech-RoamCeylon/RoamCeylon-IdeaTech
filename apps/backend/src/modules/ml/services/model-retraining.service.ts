...
export type RetrainingState = 'idle' | 'collecting' | 'training' | 'comparing' | 'success' | 'failed';
...
export class ModelRetrainingService {
  ...
  getStatus(): RetrainingStatus {
    ...
    if (this.status.state !== 'idle' && this.status.state !== 'success' && this.status.state !== 'failed' && this.status.startTime) {
  ...
  triggerRetraining(): void {
    ...
    this.runPipeline().catch((err) => {
      this.handleFailure(err.message || 'Unknown error during pipeline execution');
    });
  }
  ...
  private async runPipeline(): Promise<void> {
    ...
    this.status.logs.push('[Step 1/3] Collecting training data from live database...');
    ...
    this.status.logs.push('[Step 2/3] Training Model v2 (HistGradientBoostingClassifier)...');
    ...
    this.status.logs.push('[Step 3/3] Evaluating and comparing model versions...');
    ...
    if (this.status.startTime) {
      this.status.elapsedTimeMs = Date.now() - new Date(this.status.startTime).getTime();
    }
    ...
    try {
      ...
      const reportFile = path.join(backendRoot, `MODEL_COMPARISON_REPORT_${todayStr}.json`);
      if (fs.existsSync(reportFile)) {
        const reportContent = fs.readFileSync(reportFile, 'utf-8');
        this.status.comparisonReport = JSON.parse(reportContent);
        this.status.logs.push(`[Retraining] Comparison report loaded successfully from ${path.basename(reportFile)}`);
      } else {
        this.status.logs.push('[Retraining] Warning: Comparison report JSON file not found.');
      }
    } catch (e: any) {
      this.status.logs.push(`[Retraining] Warning: Failed to parse comparison report: ${e.message}`);
    }
  ...
  cancelRetraining(): void {
    if (this.currentProcess) {
      ...
      this.handleFailure('Retraining was manually cancelled by the administrator.');
    }
  }
}
