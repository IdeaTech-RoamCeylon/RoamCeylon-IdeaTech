...
type UserInterestProfileRow = UserInterestProfile | null;
type DestinationCategoryScoreRow = DestinationCategoryScore;
type UserFeedbackSignalRow = UserFeedbackSignal | null;
type UserCategoryWeightRow = UserCategoryWeight;
type RecommendationLogRow = RecommendationLog;
...
export class MlPredictionService {
      ...
      proc.on('close', (code) => {
        if (code === 0) {
          try {
            ...
            reject(
              new Error(
                `Failed to parse prediction output: ${err.message}. Output was: ${stdoutData}`,
              ),
            );
          }
        } else {
          reject(
            new Error(
              `Prediction script failed with code ${code}. Error: ${stderrData}`,
            ),
          );
        }
      });
  ...
  async getMLRecommendations(
    ...
    const feedbackSignal: UserFeedbackSignalRow = await this.retryService
      .withRetry<UserFeedbackSignalRow>(
        () =>
          ...
          }),
        {
          maxAttempts: 3,
          baseDelayMs: 100,
          label: 'userFeedbackSignal.findUnique',
        },
      )
      .catch(() => null);

    // Fetch user category weights with retry
    const categoryWeights: UserCategoryWeightRow[] = await this.retryService
      .withRetry<UserCategoryWeightRow[]>(
        () =>
          ...
          }),
        {
          maxAttempts: 3,
          baseDelayMs: 100,
          label: 'userCategoryWeight.findMany',
        },
      )
      .catch(() => [] as UserCategoryWeightRow[]);

    // Fetch recommendation logs with retry to build engagement features
    const recLogs: RecommendationLogRow[] = await this.retryService
      .withRetry<RecommendationLogRow[]>(
        () =>
          ...
          }),
        {
          maxAttempts: 3,
          baseDelayMs: 100,
          label: 'recommendationLog.findMany',
        },
      )
      .catch(() => [] as RecommendationLogRow[]);
    ...
    const click_frequency =
      recLogs.length > 0
        ? recLogs.filter((l) => l.clicked).length / recLogs.length
        : 0.0;
    ...
    if (recLogs.length > 0 && recLogs[0]?.createdAt) {
      engagement_recency = Math.floor(
        (Date.now() - new Date(recLogs[0].createdAt).getTime()) /
          (1000 * 60 * 60 * 24),
      );
    }

    const totalFeedbackCount = feedbackSignal
      ? feedbackSignal.positiveCount +
        feedbackSignal.negativeCount +
        feedbackSignal.neutralCount
      : 0;

    const feedback_positivity_rate =
      totalFeedbackCount > 0 && feedbackSignal
    ...
    const userFeaturesPayload = {
      ...
      category_affinity:
        categoryWeights.length > 0 ? categoryWeights[0].weight : 1.0,
      user_trust_score,
    ...
    try {
      this.logger.log(
        `Invoking Python ML model for user ${user_id} prediction...`,
      );
      ...
      predictions.recommendations = predictions.recommendations.map((rec) => ({
        ...
        ml_score: this.boundsEnforcer.enforceMlOutputScore(
          rec.ml_score,
          rec.destination_id,
        ),
      }));
      ...
      this.logger.warn(
        `Python ML inference failed, using rule-based mock fallback. Error: ${errMsg}`,
      );
    }
