...
import { PassengerRequestDto, DriverAcceptDto, RideCancelDto } from './dto/transport-events.dto';
...
export class TransportGateway {
    ...
    handlePassengerRequest(@MessageBody() data: PassengerRequestDto) {
        ...
        return { event: 'passenger_request_ack', data: { status: 'received', timestamp: new Date().toISOString() } };
    }
    ...
    handleDriverAccept(@MessageBody() data: DriverAcceptDto) {
        ...
        return { event: 'driver_accept_ack', data: { status: 'accepted', rideId: data.rideId } };
    }
    ...
    handleRideCancel(@MessageBody() data: RideCancelDto) {
        ...
        return { event: 'ride_cancel_ack', data: { status: 'cancelled', rideId: data.rideId } };
    }
}
