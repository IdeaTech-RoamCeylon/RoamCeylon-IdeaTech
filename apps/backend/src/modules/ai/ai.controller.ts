...
export class AIController {
  ...
  async tripPlanEnhanced(
    ...
    this.logger.log(`Generating enhanced trip plan for: ${body.destination}`);
    ...
    const matchedPrefs =
      body.preferences?.filter((pref) =>
        allActivities.some(
          (a) => a.category.toLowerCase() === pref.toLowerCase(),
        ),
      ) || [];
  ...
  private readonly CATEGORY_SEQUENCE: ItineraryCategory[] = [
    'Arrival',
    'Sightseeing',
    'Culture',
    'Nature',
  ...
  private readonly ROTATION_PATTERN: ItineraryCategory[] = [
    'Sightseeing',
    'Nature',
    'Culture',
    'Beach',
  ...
  private generateItinerary(
    ...
    const priorityMap = new Map(scored.map((s) => [s.id, s.priorityScore]));
    ...
    for (let day = 1; day <= dayCount; day++) {
      ...
      const activitiesThisDay = this.calculateActivitiesForDay(
        maxActivities - activityIndex,
        dayCount - day + 1,
      );
      ...
      for (
        ...
        const seqIndex = this.CATEGORY_SEQUENCE.indexOf(category);
        category = this.CATEGORY_SEQUENCE[seqIndex >= 0 ? seqIndex : 0];
        ...
        activitiesForDay.push({
          ...
          priority: Math.round((priorityMap.get(result.id) ?? 0) * 100) / 100,
        });
  ...
  private inferCategoryFromText(text: string): ItineraryCategory | null {
  ...
  private determineActivityCategory(
    ...
    if (preferences?.length) {
      for (const pref of preferences) {
        ...
        if (text.includes(prefLower)) {
          const inferred = this.inferCategoryFromText(prefLower);
    ...
    return this.ROTATION_PATTERN[
      (dayNumber + activityIndex) % this.ROTATION_PATTERN.length
    ];
  }
  ...
  private selectDiverseActivities(
    ...
    for (const result of sorted) {
      ...
      const text =
        ...
        `${result.title} ${result.content}`.toLowerCase().trim();

      if (textSet.has(text)) continue;

      const category = this.inferCategoryFromText(text) || 'General';
  ...
  private calculateActivitiesForDay(
    remainingActivities: number,
    remainingDays: number,
  ): number {
    return Math.ceil(remainingActivities / remainingDays);
  }
  ...
  private generateDayTheme(activities: EnhancedItineraryItemDto[]): string {
    ...
    if (categories.includes('Nature')) return 'Nature & Wildlife';
