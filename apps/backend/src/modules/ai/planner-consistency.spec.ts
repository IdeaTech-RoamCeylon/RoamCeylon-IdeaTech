...
import { TripPlanRequestDto, SearchResultItem } from './ai.controller';
...
describe('AI Planner Consistency & Preferences', () => {
    ...
    describe('Preference Handling', () => {
        it('should use defaults when preferences are missing', async () => {
            ...
            const mockItem = { id: '1', title: 'Place A', content: 'Description A', score: 0.9 };
            ...
            const result = await controller.tripPlanEnhanced({ user: { id: 'test-user' }, headers: {} } as any, request);
        ...
        it('should prioritize items matching preferences', async () => {
            ...
            const historyItem = { id: '1', title: 'Ancient Temple', content: 'History and culture of the ancient temple', score: 0.8 };
            const natureItem = { id: '2', title: 'Forest Park', content: 'Trees and nature', score: 0.8 };
            ...
            const result = await controller.tripPlanEnhanced({ user: { id: 'test-user' }, headers: {} } as any, requestWithHistory);
            ...
            const planActivity = result.plan.dayByDayPlan[0].activities.find(a => a.placeName === 'Ancient Temple');

            expect(planActivity).toBeDefined();
            // The explanation should mention it matches interests
            expect(JSON.stringify(planActivity?.explanation)).toContain('matches your interest');
        });
    });

    describe('Consistency Checks', () => {
        it('should return identical plans for identical requests', async () => {
            const itemA = { id: '1', title: 'Temple', content: 'A temple', score: 0.85 };
            ...
            const result1 = await controller.tripPlanEnhanced({ user: { id: 'user-1' }, headers: {} } as any, request);
            const result2 = await controller.tripPlanEnhanced({ user: { id: 'user-1' }, headers: {} } as any, request);
