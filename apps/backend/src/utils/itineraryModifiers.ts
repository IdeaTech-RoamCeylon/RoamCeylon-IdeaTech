...
export const reorderActivity = (
  ...
  const newPlan: TripPlan = JSON.parse(JSON.stringify(currentPlan));
  ...
  if (isMorningSlot && (bestTime.includes('night') || bestTime.includes('evening'))) {
    warnings.push(`Note: ${movedItem.placeName} is usually best visited in the ${bestTime}.`);
  }
...
export const applyDelay = (
  ...
  if (delayMinutes > 120 && newPlan.destinations.length > 2) {
    ...
    if (droppedItem) {
        warnings.push(`Due to the ${delayMinutes} min delay, we removed ${droppedItem.placeName}.`);
    }
  }
 ...
 * 3. STATE EXPLAINER
 */
export const generateStateMessage = (change: ChangeType, details: string): string => {
  switch (change) {
    case 'REORDER': return `Updating route... Re-sequenced trip for ${details}.`;
    case 'DELAY': return `Delay detected. ${details} We've adjusted your stops.`;
    case 'REMOVE': return `Optimizing schedule... Removed ${details}.`;
    default: return "Updating itinerary...";
  }
};